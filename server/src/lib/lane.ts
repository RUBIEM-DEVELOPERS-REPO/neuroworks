...
const SIGNALS: HeuristicProfile = {
  ...
  sreSignal: /\b(?:incident runbook|on[- ]call (?:rotation|escalation)|p95|p99 latency|SLO|SLA|error budget|blameless postmortem|kubectl|systemctl|journalctl|prometheus|grafana|datadog|kafka(?:\s+(?:cluster|partition))?|kubernetes|container orchestration|connection pool exhausted|service mesh|microservices?\s+architecture|message broker|event(?:s?[- ]?per[- ]sec|s?\/sec)|multi[- ]tenant\s+isolation|architecture\s+for|system\s+design|ingestion\s+pipeline)\b/i,
