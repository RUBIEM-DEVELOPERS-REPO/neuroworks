...
// Animated starfield background — a canvas warp field with occasional
// glitter flashes, sitting behind all page content. Adapted from a
// Framer-marketplace component (GlitterWrap); the star physics/rendering
// loop is kept close to the original (it's carefully tuned — see the
// per-line comments below), stripped of Framer's property-control plumbing
// (RenderTarget/static-export branching never applied outside Framer's own
// canvas) and given two things the original didn't need: it pauses via the
// Page Visibility API when the tab isn't visible, and skips the RAF loop
// entirely under prefers-reduced-motion (paints one settled frame instead).
//
// Tuned down from the demo defaults (500 particles, full brightness/glitter)
// to read as ambient depth behind content, not a foreground attraction —
// this sits behind text on every page. Palette pulled from the app's own
// accent tokens (tailwind.config.js) instead of the demo's white/red/yellow.
...
export function GlitterBackground({
  // Glacier (cream-50) — the bulk of the field, matches heading text colour.
  color1 = "#d8ecf8",
  // Electric Iris (violet-400) — the app's one first-class action colour.
  color2 = "#7d57f6",
  // Ember (flame-400) — the warm secondary accent.
  color3 = "#e46d4c",
  particleCount = 160,
  speed = 2.2,
  density = 100,
  ...
  const propsRef = useRef({
    color1, color2, color3, particleCount, speed, density, starSize,
    focalDepth, turbulence, brightness, glitterIntensity, trailAmount, reverse,
  });
  propsRef.current = {
    color1, color2, color3, particleCount, speed, density, starSize,
    focalDepth, turbulence, brightness, glitterIntensity, trailAmount, reverse,
  ...
  const colorCacheRef = useRef({
    color1: "" as string, color2: "" as string, color3: "" as string,
    parsed1: [255, 255, 255, 1] as [number, number, number, number],
    parsed2: [125, 87, 246, 1] as [number, number, number, number],
    parsed3: [228, 109, 76, 1] as [number, number, number, number],
  });
  ...
  useEffect(() => {
    ...
    if (reducedMotion) {
      // One settled-looking frame, then stop — no ongoing motion for users
      // who've asked for it, but the field still reads as populated rather
      // than blank.
